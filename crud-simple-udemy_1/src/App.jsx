...
function App() {
  ...
  const agregarTarea = (e) => {
    ...
    setTareas([
      ...tareas, {id: Date.now(), nombreTarea: tarea}
    ]);
  ...
  const editarTarea = (e) => {
    ...
    const arrayModificado = tareas.map(
              ...
              tareas.length === 0 ? (
                ...
                tareas.map((item) => (
                  <li className="list-group-item" key={item.id} >
                    ...
                    <button className="btn btn-warning btn-sm  float-right" onClick={() => iniciarEditarTarea(item)}>Editar</button>
                  </li>
                ))
              )
